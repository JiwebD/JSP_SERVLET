...
<!-- 오라클라이브러리 ->  webapp/WEB-INF/lib경로에 넣어주기 -->
<!-- 속싱/기능추가 -->
<%@page import = "java.sql.*,C04.UserDto" %>
<%!
	//속성/기능 추가가능
...
<%
	...
	/* 요청 정보확인 */
/* 	String url = (String)request.getAttribute("url");	//URL 정보
	...
	if(url.contains("/myinfo")){ 
		...
		if(username==userid){
			request.setAttribute("username_msg","이미 존재하는 아이디입니다.");
			request.getRequestDispatcher("./join_form.jsp").forward(request,response);
		}
		else{
			out.println("<script>alert('회원가입에 성공하였습니다 로그인페이지로 이동합니다.');location.href='./login.jsp'</script>");
		}
	//	return ;
