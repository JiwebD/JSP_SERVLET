...
<body>
	<!-- 
		...
		2. FileSystem에 저장			->		DB 저장경로 등록		(추천)
			c:\upload\user\file.txt   		"c:\upload\user\file.txt"
		...
		/upload/seller1/가전/노트북/notebook.jpgm,2,3,
	 -->
	<h1>FILE UPLOAD</h1>
	<!-- enctype="multipart/form-data" : 파일 업로드를 하기 위해 반드시 지정해야 하는 속성 -->
...
.preview::-webkit-scrollbar {
	...
	const formData = new FormData(); //왼쪽은 소문자f  오른쪽은 대문자 F
    ...
    d4El.addEventListener('drop', (e) => {
        ...
        newImageEl.setAttribute('style','width:100%;height:100%;object-fit:contain;height:150px;padding:2px;');        //새로 생성된 노드는 .style을 사용할 수 없다.
    ...
    imageUploadBtnEl.addEventListener('click',()=>{
    	...
    	//비동기함수 (axios 사용)
    	axios 	.post(path + `/file/upload` ,formDatta,{ headers : {'Content-Type':'multipart/form-data'} })
